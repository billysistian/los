...
if($jenis_pemohon == "perorangan"){
    ...
    $data = mysql_query("SELECT * FROM permohonan_kredit_temp WHERE id_permohonan_kredit='$id'");
    $row = mysql_fetch_assoc($data);

    if($row['id_permohonan_kredit'] == $id){
        echo "Hanya bisa melakukan update data permohonan kredit setelah data sebelumnya disetujui. Silakan tunggu proses persetujuan.";
        exit;
    }
    ...
    $query = mysql_query("
    INSERT INTO permohonan_kredit_temp SET
    ...
    $permohonan_id = mysql_insert_id();

    // INSERT AGUNAN
    if(isset($_POST['jenis_agunan'])){
        foreach($_POST['jenis_agunan'] as $i=>$jenis){
            $id_agunan = isset($_POST['id_agunan'][$i]) 
                     ? $_POST['id_agunan'][$i] 
                     : '';
            $nomor = isset($_POST['nomor_agunan'][$i]) 
                     ? $_POST['nomor_agunan'][$i] 
                     : '';
            $nama_pemilik = isset($_POST['nama_pemilik'][$i]) 
                     ? $_POST['nama_pemilik'][$i] 
                     : '';
            $alamat = isset($_POST['alamat_agunan'][$i]) 
                     ? $_POST['alamat_agunan'][$i] 
                     : '';
            $nama_agunan = isset($_POST['nama_agunan'][$i]) 
                     ? $_POST['nama_agunan'][$i] 
                     : '';

            $agunan = mysql_query("
            INSERT INTO agunan_kredit_temp SET
            id_permohonan_kredit='$permohonan_id',
            id_agunan_kredit='$id_agunan',
            jenis_agunan='$jenis',
            nomor_agunan='$nomor',
            nama_pemilik='$nama_pemilik',
            nama_agunan='$nama_agunan',
            alamat_agunan='$alamat'
            ");
    ...
    mysql_query("
        ...
        VALUES('Permohonan Kredit','$id','UPDATE',NULL,'".mysql_real_escape_string(json_encode($_POST))."','$edit_by','$edit_at')
    ");
